...
fun Route.authRoutes() {
    ...
    post("/verifyOtp") {
        ...
        val response: Response<AuthResponse?> = if (users == null || users.isEmpty()) {
            ...
            if (otp != null && otp.otp == requestUser.otp && otp.phone == requestUser.phone) {
                ...
                details?.token = token
                Response(AuthResponse(details, token, details != null), "Token is sent!", Result.SUCCESS.ordinal)
            } else {
                Response(null, "OTP Verification failed", Result.ERROR.ordinal)
            }
        }
    ...
    authenticate {
        get("/verify") {
            val details = UserDetailQueries.getDetail(call.user.id)
            val response = Response(
                AuthResponse(details, details?.token, details != null),
